...
def sys_spec():
    ...
    backup = 'No'
    #Random panel spec
    [panel_brand,panel_size] = [s_panel_b[randrange(len(s_panel_b))],s_panel_s[randrange(len(s_panel_s))]]
    #Random inverter spec
    if supp == 1: #If single-phase supply
        [inv_model,inv_brand,inv_size,inv_phase] = s_spec[supp-1][randrange(len(s_spec[supp-1]))]                 
    else: #If three-phase supply
        temp_phase = choice([1,3])
        [inv_model,inv_brand,inv_size,inv_phase] = s_spec[temp_phase-1][randrange(len(s_spec[temp_phase-1]))]
    ...
    if inv_brand == 'SolarEdge': #SolarEdge
    ...
    elif inv_brand == 'Huawei' and panel_size < 450 and choices([True,False],[2,8])[0]: #Huawei
        optimiser_brand = 'Huawei'
        optimiser_model = 'SUN2000-450W-P'
        optimiser_num = 7+randrange(panel_num-6)
    else: #Other inv brand
...
#for i in range(1):
 #   pprint.pprint(sys_spec(), sort_dicts=False)
  #  print('------')